...
裸跑时用上次的答案填默认值。**记忆范围严格限定在「没有文档默认值」的参数**——`--bucket` / `--budget` / `--duration` / `--profile` / `--region` / `--dest-region` / `--endpoint-url` / `--total-size` / `--iterations` / `--max-duration`。
...
### 5.10 指标与报告
- 周期性：瞬时/累计吞吐、实际 vs 目标速率、已烧 $（含**在途**）/ 目标 $、迭代数、**重试数**、错误/SlowDown 计数、复制积压
