...
```bash
# 交互式:未配复制时会问你,默认填好随机 5 个目标区域 ≈ 5× 速度,可自行增删
yo-s3 --budget 500 --bucket my-burn-bucket

# 无人值守:没配复制就随机挑 5 个区域自动建桶配规则
yo-s3 --budget 500 --bucket my-burn-bucket --yes
...
**目标区域默认随机抽**,从 16 个默认启用的商用区域里取 5 个,源区域和半价对自动排除。为什么可以随便抽:跨区流量的费率**由源区域决定**,换哪几个目标都一样快,所以这个选择对烧钱速度和预算精度都没有影响 —— 固定 5 个区域只会让每个跑过它的账号留下同一组指纹。opt-in 区域(af-south-1、me-south-1 等)不在池里,它们对没启用过的账户会直接失败。

`--dest-region` 可逗号分隔,也可重复传,给了就用你点的名。目标区域不能与源区域相同(同区复制不产生跨区流量),重复区域会被拒绝。

无人值守自动配置时会**无条件打印**将创建的桶名、区域、IAM 角色名和拆除命令 —— 不拦你,但 cron 日志里查得到。桶上已有复制规则时,这份清单还会写明**它们会被整体替换**(AWS 不支持只改其中一条),交互确认页和 `--yes` 日志用的是同一份清单。唯一不自动配的情况是桶上已有复制规则而你又没给 `--dest-region`:替换别人的复制配置超出了「无人值守烧我的预算」该有的权限,要改得显式点名区域。

`--dry-run` 一律不在账号里创建任何资源,但会按真跑会用的那 5 个目标计费,所以预演出来的写入量和实跑一致。同区目标、重复目标这些实跑会拒绝的参数,dry-run 同样当场拒绝 —— 预演一个跑不起来的运行没有意义。
...
所以随机抽取时会**排除与源区域构成半价对的区域**(源在 us-east-1 就不抽 us-east-2,反之亦然)——选它反而因为半价而拖慢烧钱。判据是查价格表而不是写死区域名,将来多一对折扣当天生效。
...
- 记住之后**下次自动使用**,不用再粘也不用加 `--profile`(链上有别的可用凭据时以链为准);那组密钥失效了会说清楚,并告诉你去哪改
- 在菜单里**选过的 profile 同样会记住**(和桶名/预算一样进 `~/.yo/s3/last-run.json`),下次开头打印 `沿用上次参数: --profile <名>`,显式传参可覆盖,`rm` 那个文件即可清空
- **`--yes` 下绝不弹窗**:无人值守时如实报错退出 —— nohup 跑了一半停下来等输入比失败更糟。记住过的凭据在 `--yes` 下同样自动生效,重启接着跑
...
**自动配置跨区复制**(桶未配复制时,交互确认 / `--dest-region` / `--yes` 都会触发)需要额外的一次性较高权限(`s3:CreateBucket`、`s3:PutBucketVersioning`、`s3:PutBucketTagging`、`s3:PutReplicationConfiguration`、`iam:CreateRole`、`iam:PutRolePolicy`、`iam:GetRole`、`iam:PassRole`),建议用管理员身份先跑一次把复制配好;之后的烧钱运行只需下面的常规权限。

**`cleanup`** 需要 `s3:ListAllMyBuckets` 来反查上一轮没拆干净的目标桶(目标区域随机,桶名推不出来)。缺这个权限时打印一行跳过,不影响其余清理。
...
| `--dest-region` | 随机抽 5 个 | 跨区复制目标区域(逗号分隔)。桶未配复制时自动建目标桶+IAM角色+复制规则再开烧,不给就从 16 个默认启用区域里随机挑(费率由源区域决定,挑哪几个都一样快);已配则以现有配置为准 |
...
- **上一轮没拆干净的目标桶也会被找出来**:除了读源桶的复制规则,还会用 `ListBuckets` 按 `<源桶>-crr-` 前缀反查。目标区域是随机的,桶名推不出来,而拆除是「哪步失败报哪步、继续往下」——删了规则却没删掉桶的话,那个桶就再没有东西指向它、一直计存储费。反查只认带 `yo-s3-created` 标签且桶名能完整还原的,不会把你自己的桶拉进删除清单;复制配置读不出来时(权限不足等)整个反查会跳过,因为「没有规则指向它」这个判据此时分不清在用的和遗留的
